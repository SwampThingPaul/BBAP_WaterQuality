...
bb.nnc=subset(nnc.regions,ESTUARY=="Biscayne Bay"&ESTUARY_SE%in%paste0("ENRH",1:9))# originally 3:9
...
xlim.val=range(tmp[2][[1]]$raw[1]);by.x=0.1;xmaj=seq(xlim.val[1],xlim.val[2],by.x);xmin=seq(xlim.val[1],xlim.val[2],by.x/2);xlen=length(xmaj)
...
test=lm(log(GM.TN)~.,data=tmp)
layout(matrix(1:4,2,2));plot(test)
dev.off()

test2=MASS::boxcox(test,plotit=T,lambda=seq(-0.4,0.2,0.01))
bc.lambda=test2$x[which.max(test2$y)]

tmp$bc.GM.TN=(tmp$GM.TN^bc.lambda-1)/bc.lambda
# plot(exp(log(tmp$bc.GM.TN*bc.lambda+1)/bc.lambda)~tmp$GM.TN);abline(0,1)
# cbind(exp(log(tmp$bc.GM.TN*bc.lambda+1)/bc.lambda),tmp$GM.TN)

test=lm(bc.GM.TN~.,data=tmp)
layout(matrix(1:4,2,2));plot(test)
dev.off()
...
m.TN=bam(log(GM.TN)~
           s(WY,k=7)+
           s(Longitude,Latitude,bs="tp",k=40,m=c(1,0.5))+
           ti(Longitude,Latitude,WY,d=c(2,1),bs=c("tp","tp"),k=c(40,7)),
         data=tmp)
...
m.Chl=bam(log(GM.Chla)~
           ...
           ti(Longitude,Latitude,WY,d=c(2,1),bs=c("ds","tp"),k=c(20,5)),
         data=tmp)
...
GAM.Chl.stack2=stack(GAM.Chl.2015,
                    GAM.Chl.2016,
                    GAM.Chl.2017,
                    GAM.Chl.2018,
                    GAM.Chl.2019)
